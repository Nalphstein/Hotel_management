...
// Define the type for our category data prop
interface CategoryData {
  [key: string]: number;
}

// Define the prop types for the component
interface SalesCategoriesCardProps {
...
const SalesCategoriesCard = ({ categoryData }: SalesCategoriesCardProps) => {
  const { user } = useAuth(); // Get the current authenticated vendor from our context
  
  // --- State Management ---
  const [timeframe, setTimeframe] = useState('Weekly');
  const [localCategoryData, setLocalCategoryData] = useState<any[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  // Use categoryData if provided, otherwise use the existing data fetching logic
  useEffect(() => {
    if (categoryData) {
      // Transform the provided categoryData into the format needed for the chart
      const transformedData = Object.entries(categoryData).map(([name, value], index) => ({
        name,
        value,
        color: COLORS[index % COLORS.length],
      })).sort((a, b) => b.value - a.value); // Sort from largest to smallest category
      
      setLocalCategoryData(transformedData);
      setIsLoading(false);
      return;
    }
    
    // Original data fetching logic when no categoryData is provided
    if (!user) return;
    ...
    const fetchCategoryData = async () => {
      ...
      try {
        ...
        setLocalCategoryData(formattedChartData);
    ...
    fetchCategoryData();
  }, [user, timeframe, categoryData]); // Add categoryData to dependencies
        ...
        <div className="flex flex-col items-center justify-center mt-4">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {localCategoryData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
            ) : null}
          ...
          <div className="w-full mt-4 space-y-2">
            {isLoading ? (
              ...
              localCategoryData.map((item, index) => (
