...
export default function ProductDetailClient() {
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      // Add review to Firestore subcollection
      ...
      <div className= "flex" >
      {
        [1, 2, 3, 4, 5].map((star) => (
          <button
            key= { star }
            type = "button"
            className = {`text-2xl ${onRatingChange ? 'cursor-pointer' : 'cursor-default'} ${star <= (hoverRating || rating)
              ? 'text-yellow-400'
              : 'text-gray-300'
            }`}
...
className = {`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg border transition-colors text-sm sm:text-base ${selectedOptions[optionType] === option.id
