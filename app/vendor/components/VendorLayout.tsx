...
import { SearchIcon, BellIcon, UserIcon, MenuIcon, XIcon } from 'lucide-react';
...
const VendorLayout = ({ children }: VendorLayoutProps) => {
  ...
  const [isClient, setIsClient] = useState(false); // To handle SSR issues
  ...
  useEffect(() => {
    setIsClient(true); // Set client state to true after mount
  ...
  // Don't render anything on the server side to avoid SSR issues
  if (!isClient) {
    return (
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-8">
                ...
                <nav className="hidden md:flex items-center space-x-1">
                  <Link 
                    href="/vendor" 
                    className={`px-4 py-2 text-gray-500 rounded-full font-medium text-sm flex items-center`}
                  >
                    <span className="mr-2">
              ...
              {/* Profile menu and notification icons - hidden on mobile */}
              ...
              {/* Hamburger Menu Button (Mobile) - moved to the extreme end */}
                    ...
                    <img 
                      ...
                      onError={(e) => {
                        ...
                        if (target.parentElement) {
                          ...
                          parent.innerHTML = '';
                          const textNode = document.createTextNode(vendorInitials || 'JD');
          ...
          {isMenuOpen && (
            <div className="md:hidden bg-white border-t border-gray-200 py-2">
              <div className="px-2 space-y-1">
                ...
                <Link 
                  href="/vendor" 
                  className={`block px-4 py-2 rounded-md text-base font-medium ${isActive('/vendor') && !isActive('/vendor/products') && !isActive('/vendor/orders') ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100'}`}
                  onClick={handleNavigation}
                >
                  Dashboard
                </Link>
                <Link 
                  href="/vendor/products" 
                  className={`block px-4 py-2 rounded-md text-base font-medium ${isActive('/vendor/products') ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100'}`}
                  onClick={handleNavigation}
                >
                  Products
                </Link>
                <Link 
                  href="/vendor/orders" 
                  className={`block px-4 py-2 rounded-md text-base font-medium ${isActive('/vendor/orders') ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100'}`}
                  onClick={handleNavigation}
                >
                  Orders
                </Link>
